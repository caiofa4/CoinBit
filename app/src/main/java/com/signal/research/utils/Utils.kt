...
fun shareSignal(context: Context) {
    val sendIntent = Intent().apply {
        action = Intent.ACTION_SEND
        putExtra(
            Intent.EXTRA_TEXT,
            "Track 100's of crypto currencies on 150+ exchanges completely free, secure and offline. " +
                "\n Download CoinBit at: https://play.google.com/store/apps/details?id=com.signal.research"
        )
        type = "text/plain"
    }
...
fun rateSignal(context: Context) {
    val intent = Intent(Intent.ACTION_VIEW).apply {
        data = Uri.parse(
            "https://play.google.com/store/apps/details?id=com.signal.research"
        )
